 ...
 * @author Dell
 */
public class HrViewAllParticipantsResultFrame extends javax.swing.JFrame {

    private DefaultTableModel model;
    private List<ParticipantsPojo> AllParticipantsList;
    public HrViewAllParticipantsResultFrame() {
        initComponents();
    ...
    {
        try{
            AllParticipantsList=ResultDAO.ViewAllPtResult();
            for(ParticipantsPojo viewAllPtResult:AllParticipantsList){
                Vector<String> row=new Vector<>();
                row.add(viewAllPtResult.getId());
                row.add(viewAllPtResult.getJobId());
                row.add(viewAllPtResult.getJobTitle());
                row.add(viewAllPtResult.getName());
                row.add(String.valueOf(viewAllPtResult.getPercentage()));
                model.addRow(row);
            }
    ...
    private void initComponents() {
        ...
        tbleAllPt.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "P-ID", "JobId", "Jobtitle", "Name", "Percentage"
            }
        ) {
        ...
        jPanel1.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(310, 300, 650, 310));
        ...
        BtnSelectedStudents.setBackground(new java.awt.Color(204, 204, 255));
        BtnSelectedStudents.setText("Select Students");
        BtnSelectedStudents.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                BtnSelectedStudentsActionPerformed(evt);
            }
        });
        jPanel1.add(BtnSelectedStudents, new org.netbeans.lib.awtextra.AbsoluteConstraints(390, 675, 190, 30));
    ...
    private void BtnSelectedStudentsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSelectedStudentsActionPerformed
        int row=tbleAllPt.getSelectedRow();
        if(row==-1){
            JOptionPane.showMessageDialog(null,"Please Select a row First!");
            return;
        }  
//          ResultPojo rs=new ResultPojo();
//          rs.setId(CurrentUser.getId());
//          rs.setJobId(tbleAllPt.getValueAt(row,0).toString().trim());
//          rs.setPercentage(rs.getPercentage());
//          rs.setSelectedByHr("Yes");
          try{
              String pId=(String)tbleAllPt.getValueAt(row,0);
              String jobId=(String)tbleAllPt.getValueAt(row,1);
              Boolean response=ResultDAO.setSelectedByHr(pId,jobId);
                if(response){
                    ParticipantsPojo ps=ResultDAO.FetchData(pId);
                    JOptionPane.showMessageDialog(null,"Student Selected Succesfully !!!","Successful...",JOptionPane.INFORMATION_MESSAGE);
                    InternetAddress[]recipients=InternetAddress.parse(String.valueOf(ps.getUserId()+""));
                    Message m=SendMails.Sends();
                    m.setRecipients(Message.RecipientType.TO,recipients);
                    m.setSubject( "Interview Invitation From "+CurrentUser.getId());
                    m.setText("Hello "+ps.getName()+", \n\nWarm welcome from "+CurrentUser.getId()+"\n"
                             +"You are qualified our first technical round.\n"
                             +"\nJob Description -\n"
                             +"Job Id: "+ps.getJobId()+"\"\n" 
                             +"Job Title: "+ps.getJobTitle()+"\"\n"
                             +"\nIn Your Interview will be taken by "+CurrentUser.getName()+"\n" 
               ...
               JOptionPane.showMessageDialog(null,"Could not Select Student!", "UnSuccessful!",JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(HrViewAllParticipantsResultFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
